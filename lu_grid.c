...
void request_loop(int nreq){
  ...
  for(int i = 0; i < nreq; i++) {
    ...
    populate1Dmatrix(args, i, world_rank, 265);
    // printArray(args, 20);
    MPI_Send(&args, 70228, MPI_DOUBLE, 0, TAG, req_comm);
    // free1D(args);
  }
  while (1){}
}
...
void process_loop(){
  ...
  if (world_rank == 10) {
    while (1) {
      MPI_Recv(&args, 70228, MPI_DOUBLE, 0, TAG, proc1_comm, &status);
    ...
    while (1) {
      MPI_Recv(&args, 70228, MPI_DOUBLE, 0, TAG, proc2_comm, &status);
      printf("\t\t\tPROCESSING REQ %.0f RECEIVED FROM %.0f ON PROCESSOR %d [size: %.0f]\n", args[0], args[1], world_rank, args[2]);
      kernel_lu(20, args);
      // printArray(args, 20);
    }
    ...
    while (1) {
      MPI_Recv(&args, 70228, MPI_DOUBLE, 0, TAG, proc3_comm, &status);
      printf("\t\t\tPROCESSING REQ %.0f RECEIVED FROM %.0f ON PROCESSOR %d [size: %.0f]\n", args[0], args[1], world_rank, args[2]);
      kernel_lu(20, args);
      // printArray(args, 20);
    }
  ...
  while(1){
    for (int i = 0; i < 4; i++){
      if(is_valid_connection(i)){
        if(r[i] == MPI_REQUEST_NULL){
          ...
          if(grank == 0){
            MPI_Irecv(&args[i], 70228, MPI_DOUBLE, 1, TAG, comms[world_rank][i], &r[i]);
          }
          else{
            MPI_Irecv(&args[i], 70228, MPI_DOUBLE, 0, TAG, comms[world_rank][i], &r[i]);
          }
        }
      }
    }
    ...
    for(int i = 0; i < completereqs; i++) {
      ...
      if(world_rank == 2 || world_rank == 6 || world_rank == 8){
        ...
        if(world_rank == 6){
          MPI_Isend(&args[j], 70228, MPI_DOUBLE, 1, TAG, comms[world_rank][2], &rproc);
        }
        else{
          MPI_Isend(&args[j], 70228, MPI_DOUBLE, 1, TAG, comms[world_rank][3], &rproc);
        }
        ...
        if(procflag){
          ...
          if(grank == 0){
            MPI_Send(&args[j], 70228, MPI_DOUBLE, 1, TAG, comms[world_rank][pos]);
            // MPI_Irecv(&test, 1, MPI_INT, 1, TAG, comms[world_rank][j], &r[j]);
          }
          else{
            MPI_Send(&args[j], 70228, MPI_DOUBLE, 0, TAG, comms[world_rank][pos]);
